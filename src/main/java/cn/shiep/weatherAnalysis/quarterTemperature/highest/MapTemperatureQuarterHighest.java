 ...
 * @Description
 */
public class MapTemperatureQuarterHighest extends Mapper<Object, Text,Text, DoubleWritable> {
    @Override
    protected void map(Object key, Text value, Mapper<Object, Text, Text, DoubleWritable>.Context context) throws IOException, InterruptedException {
        ...
        DoubleWritable highTemp = new DoubleWritable(Double.parseDouble(tokens[4]));
        int monthValue = localDate.getMonthValue();
        //map<key:year-n>
        if (monthValue == 1 || monthValue == 2 || monthValue == 3){
            context.write(new Text(yearQuarter+1),highTemp);
        }else if (monthValue == 4 || monthValue == 5 || monthValue == 6){
            context.write(new Text(yearQuarter+2),highTemp);
        }else if (monthValue == 7 || monthValue == 8 || monthValue == 9){
            context.write(new Text(yearQuarter+3),highTemp);
        }else {
            context.write(new Text(yearQuarter+4),highTemp);
        }
    }
}
